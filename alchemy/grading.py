...
def average_estimates(records: List[OPR]) -> AssetEstimates:
    ...
    for i in averages:
        averages[i] = averages[i] / n_records
    return averages
...
def grade_entries(lxr: pylxr.LXR, previous_winners: List[bytes], entries: List[dict]):
    ...
    records_by_height = {}
    current_block_records: List[OPR] = []
    current_height = 0
    for e in reversed(entries):
        if current_height != e["dbheight"]:
            if 10 <= len(current_block_records):
                ...
                records_by_height[current_height] = current_block_records
    ...
    for height, records in records_by_height.items():
        ...
        records_by_height[current_height] = winners

    return current_height, records_by_height
